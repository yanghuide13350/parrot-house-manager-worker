...
export async function serializeParrot(env, row, secret, baseUrl, token) {
  ...
  return { id: row.id, breed: row.breed || '', species: row.species, ringNumber: row.ring_number, gender: row.gender, status: row.status, recordSource: row.record_source || 'PROFILE', purchaseDate: row.purchase_date || '', introductionStage: row.introduction_stage || '', birthDate: row.birth_date, ageLabel: ageLabel(row.birth_date), priceCents: row.price_cents, publicIntro: row.public_intro || '', privateNotes: row.private_notes || '', traits: parseJson(row.traits_json, {}), media, father: parseJson(row.father_snapshot_json, null), mother: parseJson(row.mother_snapshot_json, null), birthHatchingRecordId: row.birth_hatching_record_id || '', feedingPlanId: row.feeding_plan_id || '', activePairId: row.active_pair_id, mate: null, pairedAt: row.paired_at, revision: row.revision, createdAt: row.created_at, updatedAt: row.updated_at }
}
export function snapshot(row) { return { breed: row.breed || '', species: row.species, ringNumber: row.ring_number, gender: row.gender, birthDate: row.birth_date, media: rowMedia(row) } }
