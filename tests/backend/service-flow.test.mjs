...
test('supplies separate daily supplies from medicine and retain purchase details',async()=>{const env=setup();try{const supply=await executeCommand(env,'owner','supplies.create',{category:'SUPPLY',name:'小米',amountCents:3200,weight:'2kg',purchaseDate:'2026-08-20',notes:'本月口粮'},'supply:create'),medicine=await executeCommand(env,'owner','supplies.create',{category:'MEDICINE',name:'电解质',amountCents:2500,weight:'100g',purchaseDate:'2026-08-21',notes:''},'medicine:create');assert.ok(supply.id);assert.ok(medicine.id);const items=await executeRead(env,'owner','supplies.list',{},'https://api.example.com','');assert.equal(items.length,2);assert.equal(items[0].category,'MEDICINE');assert.equal(items[0].amountCents,2500);assert.equal(items[1].weight,'2kg');await assert.rejects(executeCommand(env,'owner','supplies.create',{category:'OTHER',name:'无效',amountCents:1,purchaseDate:'2026-08-20'},'supply:invalid'),error=>error.code==='VALIDATION_ERROR')}finally{env.DB.close()}})
